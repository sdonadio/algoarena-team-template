"""
sim/session.py — full end-to-end AlgoArena simulation, no network required.
...
Usage as a script:
    make sim
    python tests/sim_session.py       (a thin shim that calls main() below)
...
Why the module lives here and not in tests/
-------------------------------------------
It used to be tests/sim_session.py only, and `from tests.sim_session import
SimSession` broke on a lot of machines: `tests/` has no `__init__.py`, so it
is a namespace package, and any third-party regular `tests` package in
site-packages (several ML libraries ship one) shadows it no matter what order
sys.path is in.  `sim/` is a real package with an `__init__.py`, so
`from sim.session import SimSession` cannot be shadowed.

tests/sim_session.py is still there and still works — it re-exports every
public name in this module and keeps its `python tests/sim_session.py` entry
point — so all three historical import forms behave identically:
...
applies to this file: it must never open a socket.
"""
...
class SimulatedExchange:
    ...
    def __init__(
        ...
        import plugins.securities.synthetic  # noqa: F401  (registers SYNTH)
...
# ─────────────────────────────────────────────────────────────────────────────
# main() — the `make sim` demonstration run
# ─────────────────────────────────────────────────────────────────────────────

def main() -> SessionResult:
    """Run the `make sim` demo: 3,600 ticks, 3 traders, SYNTH + AAPL.

    Called by both `python -m sim.session` and `python tests/sim_session.py`.
    Returns the SessionResult so a caller can assert on it.
    """
    ...
    # SYNTH is a deterministic sine-plus-drift wave (period 3 600 ticks,
    # amplitude ±$10 around $100, +1%/cycle) — see plugins/securities/synthetic.py.
    ...
    print("(3 600 ticks = one full sine cycle on SYNTH: prices swing "
          "$100→$110→$91→$101)\n")
